...
namespace IdentityServerHost.Quickstart.UI
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> LoginTwoStep(string email, bool rememberLogin, string returnUrl)
        {
            ...
            var message = new Message(new string[] {email}, "Authenticate token", token, null);
        ...
        private async Task HandleLockout(string modelUsername, string modelReturnUrl)
        {
            var user = await _userManager.FindByEmailAsync(modelUsername);
            var forgotPassLink = Url.Action(nameof(ForgotPassword), "Account", new {modelReturnUrl}, Request.Scheme);

            var content =
                string.Format(@"Your account is locked out, to reset you password, please click this link: {0}",
                    forgotPassLink);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Register(UserRegistrationModel userRegistrationModel, string returnUrl)
        {
            if (!ModelState.IsValid)
            {
